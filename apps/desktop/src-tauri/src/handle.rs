...
impl ClaimRequirements {
    /// Create requirements check with current stats
    /// Requirements: 100 breadcrumbs, 20.0 trust score
    ...
    /// Get percentage complete for breadcrumbs
    pub fn breadcrumb_progress(&self) -> f64 {
        (self.breadcrumbs_current as f64 / self.breadcrumbs_required as f64 * 100.0).min(100.0)
    }
    
    /// Get percentage complete for trust
    pub fn trust_progress(&self) -> f64 {
        (self.trust_current / self.trust_required * 100.0).min(100.0)
    }
}
    ...
    match value {
        ...
        serde_json::Value::Object(obj) => {
            ...
            let pairs: Vec<String> = keys
                .iter()
                .filter(|k| !obj[*k].is_null()) // Filter out null values
