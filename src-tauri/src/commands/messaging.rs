...
use tauri::State;
use gns_crypto_core::{GnsIdentity, create_envelope_with_metadata};
use crate::AppState;
...
pub async fn send_message(
    ...
    let (recipient_pk, recipient_enc_key) = if let Some(handle) = recipient_handle {
        ...
        let info = state.api.resolve_handle(&handle).await
        ...
        let info = state.api.get_identity(&pk).await
...
pub async fn get_threads(
    ...
    let threads = db.get_threads(include_archived.unwrap_or(false), limit.unwrap_or(50))
...
pub async fn get_messages(
    ...
    let db = state.database.lock().await;
    let messages = db.get_messages(&thread_id, limit.unwrap_or(50), before_id.as_deref())
        .map_err(|e| e.to_string())?;
